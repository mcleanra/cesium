define([
    ...
    function GroundPrimitive(options) {
        ...
        if (!defined(appearance) && defined(geometryInstances)) {
            var geometryInstancesArray = isArray(geometryInstances)
                ? geometryInstances
                : [geometryInstances];
        ...
        this._classificationPrimitiveOptions = {
            ...
            vertexCacheOptimize: defaultValue(
    ...
    function getRectangle(frameState, geometry) {
        ...
        for (var i = 0; i < length; i += 3) {
            ...
            var lowPosition = Cartesian3.unpack(
                lowPositions,
                i,
    ...
    function createBoundingVolume(groundPrimitive, frameState, geometry) {
        ...
        if (rectangle.width < CesiumMath.PI) {
            ...
            var highPositions = geometry.attributes.position3DHigh.values;
            var lowPositions = geometry.attributes.position3DLow.values;
            groundPrimitive._boundingVolumes.push(
    ...
    function updateAndQueueCommands(
        ...
        if (passes.pick) {
            ...
            for (var j = 0; j < pickLength; ++j) {
                ...
                if (!groundPrimitive._useFragmentCulling) {
                    var pickOffset =
                        pickOffsets[boundingVolumeIndex(j, pickLength)];
    ...
    GroundPrimitive.prototype.update = function(frameState) {
        ...
        if (
            ...
            !(this.appearance instanceof PerInstanceColorAppearance)
        ) {
            throw new DeveloperError(
                "GroundPrimitives with Materials can only classify ClassificationType.TERRAIN at this time."
            );
        }
        //>>includeEnd('debug');
        ...
        if (!defined(this._primitive)) {
            ...
            var instances = isArray(this.geometryInstances)
            ...
            var useFragmentCulling =
                GroundPrimitive._supportsMaterials(frameState.context) &&
                this.classificationType === ClassificationType.TERRAIN;
